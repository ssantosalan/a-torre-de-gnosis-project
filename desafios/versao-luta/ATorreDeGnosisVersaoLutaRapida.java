...
public class ATorreDeGnosisVersaoLutaRapida {
	static Scanner entrada = new Scanner(System.in); //variável global de entrada
	static int hpUsuario = 71, chamasNegras = 6, elixirDaVida = 10, sabedoriaDeOdin = 5; //vida global do protagonista e habilidades
	static int temp_luta = 50, temp_dialog = 0, temp_questions = 0, temp_dialogSlow = 200, temp_dialogFast = 80, temp_dialogFaster = 30, ultra_Speed = 10,  temp_dotsSlow = 0, temp_dots = 0;
	
	//vida global do protagonista e habilidades
	//hpUsuario = 100, chamasNegras = 10, elixirDaVida = 15, sabedoriaDeOdin = 5; 
	...
	//temp_luta = 50, temp_dialog = 50, temp_questions = 15, temp_dialogSlow = 200, temp_dialogFast = 80, temp_dialogFaster = 30, ultra_Speed = 10,  temp_dotsSlow = 400, temp_dots = 200;
	...
	static void ataqueDeAntonDeus(int hpAntonBossFight) throws InterruptedException {
		...
		if (hpAntonBossFight > 0) { 
			...
			if (valor == 1 || valor == 2) {
				System.out.println();
				Digita("Asulf desvia do ataque de Anton! Dano 0", TimeUnit.MILLISECONDS, temp_luta);
			} else if (valor == 3 || valor == 4 || valor == 5 || valor == 6) {
				System.out.println();
				dano = ataqueForca.nextInt(5) + 8;
				hpUsuario = hpUsuario - dano;	
				Digita("Anton atira esferas enérgicas de luz! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);						
				...
				hpUsuario = hpUsuario - dano;
				Digita("Anton voa para o céu e, com uma velocidade extrema, atinge-o diretamente! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);	
			} else if (valor == 10) {
				System.out.println();
		...
		do {
			...
			case "2":
				...
				do {
					...
					System.out.println("Você terá que informar ou '1' ou '2' ou '3' ou '4' ou '5' ou '6' ou '7' de acordo com as opções.");
	...
	public static void capitulo1() throws InterruptedException{
		...
		do {
			...
			case "1":
				...
				if (valor == 0 || valor == 1 || valor == 2 || valor == 3) {
					...
					Digita("Sacando a sua espada, você acerta um corte no joelho dele, que cai sangrando e o deixando exposto para um golpe final nas costas! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} else if (valor == 4) {
					dano = ataqueForca.nextInt(50) + 100;
					hpMonstro01 = hpMonstro01 - dano;
					Digita("Sacando a sua espada, você acerta um golpe fatal em seu pescoço, fazendo-o sangrar até a morte! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} else if (valor == 5) {
		...
		do {
			...
			if (valor == 1) {
				Digita("Ele tenta acertá-lo no peito com a ponta de sua lança, mas você desvia! Dano 0 ", TimeUnit.MILLISECONDS, temp_luta);
			} else if (valor == 2) {
			...
			case "1":
				...
				if (valor == 0 || valor == 1 || valor == 2 || valor == 3) {
					...
					Digita("Você atinge as pernas do cavalo com a espada, derrubando-os e deixando-os fora de combate! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} else if (valor == 4) {
					dano = ataqueForca.nextInt(50) + 100;
					hpMonstro02 = hpMonstro02 - dano;
					Digita("Você acerta um golpe vertical embaixo o braço que segurava a lança, fazendo-o cair do morto! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} 
				System.out.println();
				if (hpMonstro02 <= 0) {
					vitoria = true;
		...
		do {
			...
			if (contador == 0) {
				...
				Digita("Você desvia do ataque! Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);	
			} else if (contador == 1) {
				Digita("Siegfried se vira girando as espadas em sua direção! \n", TimeUnit.MILLISECONDS,
						temp_luta);
				Digita("Mas Asulf abaixa! Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);
			} else if (contador == 2) {
				Digita("Siegfried tenta mais uma vez lhe acertar com um golpe horizontal! \n", TimeUnit.MILLISECONDS, temp_luta);
				Digita("Mas Asulf desvia pulando para trás. Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);
			...
			case "1":
				...
				if (contador == 0) {
					...
					Digita("Este é o primeiro ataque que você percebe que causa uma verdadeira perfuração na armadura de dragão dele. Dano 20 ", TimeUnit.MILLISECONDS, temp_luta);
				} else if (contador == 1) {
					hpMonstro04 = hpMonstro04 - 30;
					Digita("Asulf acerta outro golpe, desta vez na perna de Siegfried! Dano 30.", TimeUnit.MILLISECONDS, temp_luta);
				} else if (contador == 2) {
					...
					Digita("Fazendo-o largar as armas e cair no chão. Dano 50. ", TimeUnit.MILLISECONDS, temp_luta);
				}
				System.out.println();
				if (hpMonstro04 <= 0) {
					vitoria = true;
	...
	public static void capitulo4() throws InterruptedException {
		...
		boolean vitoria = false; 		//Utilizado quando o Usuário vence do 'Computador' e sai do loop.         
		int hpMonstro = 0; //Vida do Monge Corrompido
		Random ataqueForca = new Random(); // Força do ataque dado por uma aleatoriedade.
		int dano = 0; //dano causado pelo ataque.
		int valor; //Valor gerado pela 'aleatoriedade'.
		...
		do {
			...
			case "1":
				...
				if (valor == 0) {
					valor = ataqueForca.nextInt(10) + 1;
					if (valor == 1 || valor == 2) {
						Digita("O Monge desviou de seu ataque! Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);
					} else {
						dano = ataqueForca.nextInt(10) + 20;
						hpMonstro = hpMonstro - dano;
						Digita("Fraco! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
					}
				} else if (valor == 1 || valor == 2 || valor == 3) {
					dano = ataqueForca.nextInt(10) + 30;
					hpMonstro = hpMonstro - dano;
					Digita("O seu ataque não foi tão eficaz! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} else if (valor == 4) {
					dano = ataqueForca.nextInt(10) + 50;
					hpMonstro = hpMonstro - dano;
					Digita("O seu ataque foi eficaz! Dano " + dano + ".", TimeUnit.MILLISECONDS, temp_luta);
				} else if (valor == 5) {
				...
				if (hpMonstro <= 0) {
					...
					if (valor == 1 || valor == 2 || valor == 3 || valor == 4 || valor == 5 || valor == 6
							|| valor == 7) {
						Digita("Asulf desvia do ataque da Centopeia!", TimeUnit.MILLISECONDS, temp_luta);
						...
						Digita("Monge usou Chamas Corrompidas! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);
						System.out.println();
					}
				}
				if (hpUsuario <= 0) {
			...
			case "2":
				...
				if (chamasNegras > 0) {
					...
					if (valor == 0) {
						...
						Digita("CHAMAS NEGRAS! Dano 100", TimeUnit.MILLISECONDS, temp_luta);
					} else {
						hpMonstro = hpMonstro - 200;
						Digita("CHAMAS NEGRAS! Dano CRÍTICO! 200", TimeUnit.MILLISECONDS, temp_luta);
					}
				} else {
				...
				if (hpMonstro <= 0) {
					...
					if (valor == 1 || valor == 2 || valor == 3 || valor == 4 || valor == 5 || valor == 6
							|| valor == 7) {
						Digita("Asulf desvia do ataque das Chamas Corrompidas do Monge!", TimeUnit.MILLISECONDS, temp_luta);
					} else if (valor == 8 || valor == 9) {
						dano = ataqueForca.nextInt(10) + 10;
						hpUsuario = hpUsuario - dano;
						System.out.println();
						...
						Digita("Centopeia Corrompida te ataca com força! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);
						System.out.println();
			...
			case "1": //Atacando a valquiria01 SIGRUN
				...
				case "1": //Ataque Cortante
					...
					if (hpValquiria01 > 0) {
						...
						if (valor == 0 || valor == 1 || valor == 2) {
							valor = ataqueForca.nextInt(10) + 1;
							if (valor == 1 || valor == 2) {
								Digita("Sigrun desviou de seu ataque! Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);
							} else {
								dano = ataqueForca.nextInt(10) + 20;
						...
						Digita("Asulf está atancando uma Valquíria morta! ASULF ESTÁ ENLOUQUECENDO!?", TimeUnit.MILLISECONDS, temp_luta);
					}
					...
					if (hpValquiria01 <= 0 && hpValquiria02 <= 0 && hpValquiria03 <= 0) {
						...
						if (hpValquiria01 > 0) { 
							valor = ataqueForca.nextInt(10) + 1;
							if (valor == 1 || valor == 2) {
								Digita("Asulf desvia do ataque de Sigrun!", TimeUnit.MILLISECONDS, temp_luta);
							} else if (valor == 3 || valor == 4 || valor == 5 || valor == 6) {
								dano = ataqueForca.nextInt(5) + 5;
								hpUsuario = hpUsuario - dano;	
								Digita("|Sigrun| atira à distância suas penas! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);						
							} else if (valor == 7|| valor == 8) {
								dano = ataqueForca.nextInt(5) + 20;
								hpUsuario = hpUsuario - dano;								
						...
						if (hpValquiria02 > 0) {
							...
							if (valor == 1 || valor == 2 || valor == 3 || valor == 4 || valor == 5 || valor == 6) {
								dano = ataqueForca.nextInt(5) + 8;
								hpUsuario = hpUsuario - dano;	
								Digita("|Olrun| te ataca com velocidade! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);							
							} else {
								Digita("Asulf desvia do ataque de Olrun!", TimeUnit.MILLISECONDS, temp_luta);
							}
						}
						System.out.println();
						System.out.println();
						if (hpValquiria03 > 0) { //ataque valquiria03
							...
							if (valor == 1 || valor == 2 || valor == 3 || valor == 4) {
								...
								Digita("|Eir| te acerta com seu escudo! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);
								
							} else {
								Digita("Asulf desvia do ataque de Eir!", TimeUnit.MILLISECONDS, temp_luta);
							}
						}
						System.out.println();
						System.out.println();
					...
					if (hpUsuario <= 0) {
						...
						Digita("Fim de jogo!", TimeUnit.MILLISECONDS, temp_luta);
						System.exit(0);
					}
					System.out.println();
					System.out.println();
				...
				case "2": //ataque das Chamas Negras
					...
					if (hpValquiria01 > 0) {
						...
						Digita("ASULF TENTOU USAR CHAMAS NEGRAS EM UMA VALQUÍRIA MORTA! ASULF ESTÁ ENLOUQUECENDO?!", TimeUnit.MILLISECONDS, temp_luta);
						if (chamasNegras > 0) {
							chamasNegras--;
						}
					}
					...
					if (hpUsuario <= 0) {
						Digita("Derrotado pelas Valquírias! Fim de jogo!", TimeUnit.MILLISECONDS, temp_luta);
						System.exit(0);
					}
					System.out.println();
					break;
				...
				case "4":
					...
					if (hpValquiria01 > 0) {
						...
						if (sabedoriaDeOdin > 0) {
							...
							if (valor == 0) {
								hpValquiria01 = hpValquiria01 - 100;
								Digita("ODIN!!!! Dano 100", TimeUnit.MILLISECONDS, temp_luta);
							} else {
								hpValquiria01 = hpValquiria01 - 200;
								Digita("ODIN!!!! Dano CRÍTICO! 200", TimeUnit.MILLISECONDS, temp_luta);
							}
						} else {
							hpUsuario = hpUsuario - 5;
						...
						Digita("ASULF ESTÁ ATACANDO UMA VALQUÍRIA MORTA! ESTÁ ENLOUQUECENDO?!", TimeUnit.MILLISECONDS, temp_luta);
						if (sabedoriaDeOdin > 0) {
							sabedoriaDeOdin--;
						}
					}
			...
			case "2": //Atacando a valquiria02 OLRUN
				...
				case "1": //Ataque Cortante
					...
					if (hpValquiria02 > 0) {
						...
						if (valor == 1 || valor == 2 || valor == 3 || valor == 4) {					
							Digita("Olrun desviou de seu ataque! Ela é muito rápida! Dano 0. ", TimeUnit.MILLISECONDS, temp_luta);				
						...
						Digita("ASULF ESTÁ ATACANDO UMA VALQUÍRIA MORTA! ASULF ESTÁ ENLOUQUECENDO?!", TimeUnit.MILLISECONDS, temp_luta);
					}
					System.out.println();
					System.out.println();
				...
				case "1": //Ataque Cortante
					...
					if (hpValquiria03 > 0) {
						...
						if (valor == 1 || valor == 2 || valor == 3 || valor == 4 || valor == 5 || valor == 6) {	
							...
							Digita("Asulf atacou Eir! Ela é bem resistente! Dano " + dano, TimeUnit.MILLISECONDS, temp_luta);				
						} else if (valor == 7 || valor == 8 || valor == 9) {
							Digita("O seu ataque foi bloqueado pelo escudo de Eir! Dano 0.", TimeUnit.MILLISECONDS, temp_luta);
						} else if (valor == 10) {
						...
						Digita("ASULF ESTÁ ATACANDO UMA VALQUÍRIA MORTE! ASULF ESTÁ ENLOUQUECENDO!?", TimeUnit.MILLISECONDS, temp_luta);
					}
			...
			case "2": //ataque das Chamas Negras
				...
				if (chamasNegras > 0) {
					...
					if (contadorChamasNegras == 0) {
						...
						Digita("O impacto é tão grande que nem a áurea de luz branca de Anton é capaz de \n resistir à escuridão do guerreiro de Ereshkigal! Dano " + dano +"\n", TimeUnit.MILLISECONDS, temp_luta);
						dano = 20;
						hpUsuario = hpUsuario - dano;
						System.out.println();
